defmodule Orchestra.Git.ClonerTest do
  ...
  setup do
    ...
    Mox.stub_with(Orchestra.Utils.SystemMock, Orchestra.Application.system())
  ...
  describe "clone/3" do
    test "successfully clones specific workflow path", %{git_url: url, workflow_path: path, dest_path: dest} do
      ...
      expect(Orchestra.Utils.SystemMock, :cmd, fn "git", ["remote", "add", "-f", "origin", ^url], opts ->
        assert_command_opts(opts, dest)
        {"", 0}
      end)

      expect(Orchestra.Utils.SystemMock, :cmd, fn "git", ["config", "core.sparseCheckout", "true"], opts ->
        assert_command_opts(opts, dest)
        {"", 0}
      end)
